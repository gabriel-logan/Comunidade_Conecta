...
export default function JurosComposto() {
  const escolheJuroComposto = () => {
    const compostoEscolhe = document.querySelector('#composto_escolhe-calc');
    const juroComposto = document.querySelector('.calc_juros_composto');
    const capitalInicial = document.querySelector('.calc_capital_inicialcomposto');
    const taxaDeJuros = document.querySelector('.calc_taxa_juroscomposto');
  ...
  const calculaCapitalInicial = (e) => {
    ...
    // eslint-disable-next-line max-len
    const calculoCapital = Number(montanteCapital) / ((1 + (Number(jurosTaxa) / 100)) ** Number(jurosTempo));

    setCalculaCapital(calculoCapital);
  };
  const calculaTaxaDeJuros = (e) => {
    e.preventDefault();
    let errors = false;
    ...
    // eslint-disable-next-line max-len
    const calculoTJuros = ((((Number(montanteTaxa) / Number(capitalTaxa)) ** (1 / Number(taxaTempo))) - 1) * 100);
    if (errors) return;
      ...
      <div className="calc_juros_composto off">
        <form onSubmit={calculaJurosComposto}>
          <label htmlFor="none">
            ...
            <input placeholder="Ex: 50" value={capitalJuros} onChange={(e) => { setCapitalJuros(e.target.value); }} />
          </label>
          <label htmlFor="none">
            Digite o valor da Taxa de juros

            <input placeholder="Ex: 11" value={taxaJuros} onChange={(e) => { setTaxaJuros(e.target.value); }} />
          </label>
          <label htmlFor="none">
            Digite o valor do Tempo

            <input placeholder="Ex: 5" value={tempoJuros} onChange={(e) => { setTempoJuros(e.target.value); }} />
          </label>
        ...
        <div>
          <h2>
            JUROS É
            {' '}
            {calculoJurosComposto.toFixed(2)}
            {' '}
            e o
            MONTANTE
            {' '}
      ...
      <div className="calc_capital_inicialcomposto off">
        <form onSubmit={calculaCapitalInicial}>
          <label htmlFor="none">
            ...
            <input placeholder="Ex: 540" value={montanteCapital} onChange={(e) => setMontanteCapital(e.target.value)} />
          </label>
          <label htmlFor="none">
            Digite o valor da Taxa de juros

            <input placeholder="Ex: 11" value={jurosTaxa} onChange={(e) => { setJurosTaxa(e.target.value); }} />
          </label>
          <label htmlFor="none">
            Digite o valor do Tempo

            <input placeholder="Ex: 5" value={jurosTempo} onChange={(e) => { setJurostempo(e.target.value); }} />
          </label>
          <button type="submit">Calcular</button>
        </form>
        <div>
      ...
      <div className="calc_taxa_juroscomposto off">
        <form onSubmit={calculaTaxaDeJuros}>
          <label htmlFor="none">
            ...
            <input placeholder="Ex: 50" value={capitalTaxa} onChange={(e) => { setCapitalTaxa(e.target.value); }} />
          </label>
          <label htmlFor="none">
            Digite o valor do Montante

            <input placeholder="Ex: 5,3" value={montanteTaxa} onChange={(e) => { setMontanteTaxa(e.target.value); }} />
          </label>
          <label htmlFor="none">
            Digite o valor do Tempo

            <input placeholder="Ex: 5" value={taxaTempo} onChange={(e) => { setTaxaTempo(e.target.value); }} />
          </label>
          <button type="submit">Calcular</button>
        </form>
        <div>
