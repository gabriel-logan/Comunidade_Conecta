...
export default function Rendimento() {
  ...
  function handleSubmit(e) {
    ...
    const valorTotalDepositadoMensalmente = listaDeValoresDepositadosSemTaxa.reduce((soma, i) => soma + i);
    ...
    // eslint-disable-next-line max-len
    const calcMontante = (Number(capIni) * ((1 + valorConvertidoTJuros) ** Number(tempo))) + valorDepositadoSomado;
      ...
      <form onSubmit={handleSubmit}>
        ...
        <label htmlFor="none">
          ...
          <input
            ...
            type="number"
            value={tJuros}
            onChange={(e) => { setTJuros(e.target.value); }}
          />
        ...
        {valorCalculado ? (
          <>
            ...
            <h2>
              ...
              meses, equivalente a
              {' '}
              {Number.isInteger(tempo / 12) ? (tempo / 12) : ((tempo / 12).toFixed(1)
              )}
              {' '}
              ano(s)
            </h2>
          ...
          <>
            <h2>
              Rendimento Bruto = Aguardando valores...
            </h2>
            <h2>
              {' '}
              {tempo}
